...
class Gren {
    ...
    _compareTags(range, options) {
        return this.repo._request('GET', `/repos/${this.repo.__fullname}/compare/${range.previousTag.name}...${range.tag.name}`, options);
    }
    ...
    _filterBlockPullRequest(range, pullRequest) {
        ...
        return range[1].commits && range[1].commits.indexOf(pullRequest.merge_commit_sha) > -1;
    }
    ...
    async _createReleaseRanges(releaseDates) {
        ...
        for (let i = 0; i < sortedReleaseDates.length - 1; i++) {
            ...
            const sinceTag = sortedReleaseDates[i + 1].tag;
            const untilTag = sortedReleaseDates[i].tag;
            const {data: {commits}} = await this._compareTags({previousTag: sinceTag, tag: untilTag}, {per_page: 100, page: 1});
            ranges.push([
    ...
    async _getReleaseBlocks() {
        ...
        const releaseRanges = await this._createReleaseRanges(releaseDates);
